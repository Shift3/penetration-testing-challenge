...
const TABLENAMERPLINK = 'roles_permissions';
...
class MigrationFile {
  ...
  async up(transaction) {
    ...
    for ( let i = 0; i < roles.length; i++ ) {
      const role = roles[ i ];
      const script =
        `INSERT INTO ${SCHEMANAME}.${TABLENAMEROLE} (role, description) VALUES ('${role.role}', '${role.description}') RETURNING id`;
      const id = await transaction.one( script );
      roleIds.push( id );
    }
    ...
    for ( let i = 0; i < permissions.length; i++ ) {
      ...
      const script =
        `INSERT INTO ${SCHEMANAME}.${TABLENAMEPERMISSION} (permission, description) VALUES ('${per.permission}', '${per.description}') RETURNING id`;
      const id = await transaction.one( script );
      permissionIds.push( id );
    }
    // generate scripts for linking table and return
    let script = `INSERT INTO ${SCHEMANAME}.${TABLENAMERPLINK} (roles_id, permissions_id) VALUES `
