...
const migrationFolder = process.env.MIGRATION_FILES_PATH || '/home/node/app/migrations/';
...
class Migration {
    async run() {
        ...
        const query = `SELECT * FROM ${SCHEMANAME}.${MIGRATIONTABLE} ORDER BY id ASC`;
        db.any( query )
            .then( dbMigrations => {
    ...
    async executeFile( file ) {
        ...
        await db.tx( async transaction => {
            const scripts = await scriptFile.up(transaction);
            console.log( scripts );
            for ( i = 0; i < scripts.length; i++ ) {
                const sql = scripts[ i ];
                console.log( sql );
                const res = await transaction.any( sql );
                console.log( res );
            }
        } );
        return i;
    ...
    async completeMigration( file ) {
        const query2 = `INSERT INTO ${SCHEMANAME}.${MIGRATIONTABLE} (migration_name) VALUES ($1)`;
